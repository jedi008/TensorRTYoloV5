...
__global__ void find_the_max_class_score_kernel(float* output, float* cuda_p_indexs)
{
    //int i = threadIdx.x;
    float class_score = -999;
    cuda_p_indexs[0] = 2;

    //int outputBoxInfo = 80;
    //for (int k = 0; k < outputBoxInfo - 5; k++)
    //{
    //    float score = output[5 + k];
...
cudaError_t find_the_max_class_score(float* output, float* class_index, unsigned int size)
{
    ...
    find_the_max_class_score_kernel << <1, 1 >> > (output, cuda_p_indexs);
